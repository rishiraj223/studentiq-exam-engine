...
export default function OfflineTestGenerator() {
  ...
  const [generatedTestReady, setGeneratedTestReady]       = useState(false);
  ...
  // ── Generate ──────────────────────────────────────────────────────────────
  const handleGenerate = async (e?: React.FormEvent, historyTest?: SavedTest) => {
    ...
    if (!historyTest) {
      ...
      const count = mode === 'single' ? (mcqCount + numericalCount) : multiTotal;
    ...
    try {
      let finalName = testName.trim() || (mode === 'single' ? `${subject} — ${exam}` : `Mixed Paper — ${exam}`);
      let body: any;

      if (historyTest) {
        finalName = historyTest.test_name || finalName;
        body = {
          mode: 'history',
          questionIds: historyTest.question_ids,
        };
      } else {
        const count = mode === 'single' ? (mcqCount + numericalCount) : multiTotal;
        body = mode === 'single'
      ...
      if (data.questions.length < (mode === 'single' ? (mcqCount + numericalCount) : multiTotal) && !historyTest) {
        toast.warning(`Only ${data.questions.length} questions available.`);
      } else {
      ...
      const config: PDFConfig = {
        ...
        hasNumericals: data.numericalCount > 0,
        numericalCount: data.numericalCount || 0,
        numericalStartIndex: data.mcqCount > 0 ? data.mcqCount + 1 : 1,
      };
      setPdfConfig(config);
      ...
      setGeneratedTestReady(true);
      setActiveTab('builder'); // Switch back to builder tab if triggered from history
  ...
  const handleOpenHistory = async (saved: SavedTest) => {
    ...
    toast.info('Opening saved test...');
    await handleGenerate(undefined, saved);
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
                ...
                <div key={item.id} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50 transition-colors group">
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors font-semibold text-xs"
      ...
      {activeTab === 'builder' && (
        <div className="w-full">
          {!generatedTestReady ? (
            /* ── Full-Width Builder Form ───────────────────────────────── */
            <form onSubmit={e => handleGenerate(e)} className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
              
              {/* Column 1: Test Metadata & Subject Selection */}
              <div className="space-y-4">
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                  <h2 className="text-base font-bold text-slate-800 border-b border-slate-100 pb-3">Test Details</h2>
                ...
                {mode === 'single' && (
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                    <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2">
                      <BookOpen className="w-4 h-4 text-blue-500" /> Subject & Chapters
                    </h3>
                    <div>
              ...
              {/* Column 2: Question Counts, Difficulty, Duration */}
              <div className="space-y-4 flex flex-col h-full">
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-5 flex-1">
                  <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2 border-b border-slate-100 pb-3">
                    <ZapIcon className="w-4 h-4 text-blue-500" /> Test Parameters
                  </h3>
                  ...
                  {/* Question Counts - Range Sliders (Only Single Subject) */}
                  {mode === 'single' && !useMix && (
                    <div className="space-y-5 bg-slate-50 p-4 rounded-xl border border-slate-100">
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          <label className="text-xs font-bold text-slate-700 uppercase">MCQs</label>
                          <span className="text-sm font-black text-blue-600">{mcqCount}</span>
                        </div>
                        <input
                          type="range"
                          min="0" max="100"
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-2">
                          <label className="text-xs font-bold text-slate-700 uppercase">Numericals</label>
                          <span className="text-sm font-black text-blue-600">{numericalCount}</span>
                        </div>
                        <input
                          type="range"
                          min="0" max="50"
                          value={numericalCount}
                          onChange={e => setNumericalCount(parseInt(e.target.value) || 0)}
                          className="w-full accent-blue-600 h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer"
                        />
                        <div className="flex justify-between text-[10px] font-medium text-slate-400 mt-1">
                      ...
                      <div className="pt-3 border-t border-slate-200 flex items-center justify-between">
                        ...
                        <div className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-base font-black">
                  ...
                  {mode === 'multi' && (
                    <div className="bg-blue-50 p-4 rounded-xl flex items-center justify-between border border-blue-100">
                      <span className="text-sm font-bold text-slate-700">Total Questions</span>
                      <div className="px-3 py-1 bg-blue-600 text-white rounded-full text-base font-black shadow-sm">
                        {multiTotal}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="pt-2">
                    ...
                    {!useMix ? (
                      <div className="flex gap-2 flex-wrap">
                        {DIFFICULTIES.map(d => (
                          <button
                            ...
                            className={`flex-1 py-2 rounded-lg text-xs font-bold border transition-all ${difficulty === d.id ? 'bg-blue-600 text-white border-blue-600 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-300'}`}
                          >
                            {d.label}
                          </button>
                        ))}
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {(['easy', 'medium', 'hard'] as const).map(d => (
                          <div key={d} className="text-center bg-slate-50 p-2 rounded-xl border border-slate-100">
                            <label className="text-xs font-bold text-slate-500 capitalize block mb-1.5">{d}</label>
                            <input
                              type="number"
                              min="0" max="100"
                              value={diffMix[d]}
                              onChange={e => setDiffMix(prev => ({ ...prev, [d]: parseInt(e.target.value) || 0 }))}
                              className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm font-bold text-center focus:ring-2 focus:ring-blue-500 outline-none"
                            />
                          </div>
                        ))}
                      </div>
                    ...
                    {useMix && mode === 'single' && (
                      <div className="mt-3 bg-blue-50 px-3 py-2 rounded-xl text-sm text-blue-700 font-bold flex justify-between items-center">
                        <span>Total Questions (Mix):</span>
                        <span>{mixTotal}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-100">
                    <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                      <input
                        type="checkbox"
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">Skips questions used in your last 5 generated tests.</p>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 bg-blue-600 text-white font-black text-sm rounded-2xl hover:bg-blue-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-200 disabled:opacity-50 disabled:shadow-none disabled:cursor-not-allowed mt-auto"
                >
                  {isGenerating
                    ? <><Loader2 className="w-5 h-5 animate-spin" /> Fetching Questions...</>
                    : <><ZapIcon className="w-5 h-5" /> Generate Test Package</>}
                </button>
              </div>
            </form>
          ) : (
            /* ── Full-Width Download Center ────────────────────────────── */
            <div className="w-full flex flex-col gap-6 animate-in zoom-in-95 duration-300">
              {/* Success Banner + Downloads */}
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 rounded-3xl p-8 shadow-sm">
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
                  <div className="text-center md:text-left">
                    <h2 className="text-3xl font-black text-emerald-800 tracking-tight mb-2">Test Package Ready! 🎉</h2>
                    <p className="text-emerald-700 text-base font-medium opacity-90 flex items-center justify-center md:justify-start gap-2 flex-wrap">
                      <span className="bg-emerald-100 px-2 py-0.5 rounded-md font-bold">{pdfConfig?.testName}</span>
                      <span>·</span>
                      <span>{pdfConfig?.numberOfQuestions} Questions ({pdfConfig?.hasNumericals ? `${(pdfConfig.numberOfQuestions || 0) - (pdfConfig.numericalCount || 0)} MCQs + ${pdfConfig.numericalCount} Numericals` : `${pdfConfig?.numberOfQuestions} MCQs`})</span>
                      <span>·</span>
                      <span>{pdfConfig?.totalMarks} Marks</span>
                      {pdfConfig?.duration && <><span>·</span><span>{pdfConfig.duration} Mins</span></>}
                    </p>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button
                      ...
                      className="flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-700 transition-all shadow-md shadow-emerald-200 disabled:opacity-60 disabled:shadow-none hover:shadow-lg hover:-translate-y-0.5"
                    >
                      {isDownloading === 'all' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Download className="w-5 h-5" />}
                      Download All (3 PDFs)
                    </button>
                    <button
                      onClick={() => setGeneratedTestReady(false)}
                      className="flex items-center justify-center gap-2 px-6 py-3 bg-white text-slate-700 border border-slate-200 rounded-xl text-sm font-bold hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[
                    { type: 'paper' as const, label: 'Question Paper', icon: FileText, color: 'emerald', desc: 'Ready for printing' },
                    { type: 'key'   as const, label: 'Answer Key',     icon: CheckSquare, color: 'blue', desc: 'Solutions & Mapping' },
                    { type: 'omr'   as const, label: 'OMR Sheet',      icon: ListChecks, color: 'purple', desc: 'Bubble sheet' },
                  ].map(({ type, label, icon: Icon, color, desc }) => (
                    <button
                      key={type}
                      onClick={() => handleDownload(type)}
                      disabled={!!isDownloading}
                      className={`flex items-center p-5 bg-white border border-${color}-200 rounded-2xl hover:shadow-lg hover:border-${color}-400 transition-all group disabled:opacity-60 text-left`}
                    >
                      <div className={`w-14 h-14 bg-${color}-50 text-${color}-600 rounded-2xl flex items-center justify-center mr-4 group-hover:scale-110 transition-transform`}>
                        {isDownloading === type ? <Loader2 className="w-7 h-7 animate-spin" /> : <Icon className="w-7 h-7" />}
                      </div>
                      <div>
                        <h4 className="font-bold text-slate-800 text-base">{label}</h4>
                        <p className={`text-xs text-${color}-600/80 font-medium mt-0.5 flex items-center gap-1`}>
                          <Download className="w-3 h-3" /> {desc}
                        </p>
                      </div>
                    </button>
                  ))}
...
<title>${pdfConfig?.testName}</title>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/katex@0.16.8/dist/katex.min.css">
<style>body{font-family:serif;font-size:14px;padding:32px;max-width:900px;margin:0 auto;line-height:1.5} @media print{.no-print{display:none}}</style>
...
"></script>
</head><body><h2 style="text-align:center">${pdfConfig?.coachingName}</h2><h3 style="text-align:center">${pdfConfig?.testName}</h3><hr/>${html}</body></html>`);
                    win?.document.close();
                  }}
                  className="mt-6 w-full py-3 bg-white/50 border border-emerald-100 text-sm font-semibold text-emerald-700 hover:text-emerald-800 hover:bg-white rounded-xl transition-all flex items-center justify-center gap-2"
                >
                  <Printer className="w-4 h-4" /> Print via Browser (Better for Complex Equations)
                </button>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                  <h3 className="font-bold text-slate-800 flex items-center gap-2 text-base">
                    <BarChart3 className="w-5 h-5 text-blue-500" /> Preview (First 5 Questions)
                  </h3>
                  <span className="px-3 py-1 bg-slate-100 text-slate-500 rounded-full text-xs font-bold">{generatedQuestions.length} total</span>
                </div>
                <div className="p-6 space-y-4">
                  {generatedQuestions.slice(0, 5).map((q, idx) => {
                    ...
                    return (
                      <div key={q.id || idx} className="p-5 rounded-2xl bg-slate-50 border border-slate-100 hover:border-blue-100 hover:shadow-sm transition-all">
                        <div className="flex gap-3 mb-3">
                          <span className="w-7 h-7 bg-blue-600 text-white rounded-full text-xs font-bold flex items-center justify-center shrink-0 mt-0.5 shadow-sm">{idx + 1}</span>
                          <p className="text-sm text-slate-800 font-medium leading-relaxed">{qText.length > 250 ? qText.slice(0, 250) + '…' : qText}</p>
                        </div>
                        <div className="ml-10 grid grid-cols-1 md:grid-cols-2 gap-2">
                          {(q.options || []).map((opt: string, oi: number) => (
                            <span key={oi} className={`text-xs px-3 py-2 rounded-xl font-medium ${oi === q.correct_answer_index ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-white text-slate-600 border border-slate-200'}`}>
                              <span className="font-bold text-slate-400 mr-2">({labels[oi]})</span> {(opt || '').length > 60 ? opt.slice(0, 60) + '…' : opt}
                            </span>
                          ))}
                        </div>
                        <div className="ml-10 mt-3 flex items-center gap-2 text-xs">
                          <span className={`font-bold px-2.5 py-1 rounded-md ${q.difficulty === 'easy' ? 'bg-green-100 text-green-700' : q.difficulty === 'hard' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                            {q.difficulty || 'medium'}
                          </span>
                          <span className="text-slate-500 font-medium bg-white border border-slate-200 px-2.5 py-1 rounded-md truncate">{q.chapter}</span>
                          {q.marks && <span className="text-slate-500 font-bold ml-auto shrink-0 bg-slate-100 px-2.5 py-1 rounded-md">+{q.marks} / −{q.negative_marks || 1}</span>}
                        </div>
                      </div>
                    );
                  })}
                  {generatedQuestions.length > 5 && (
                    <div className="text-center pt-4 pb-2">
                      <p className="inline-flex items-center gap-2 text-sm text-blue-600 bg-blue-50 px-4 py-2 rounded-full font-bold">
                        <Layers className="w-4 h-4" />
                        + {generatedQuestions.length - 5} more questions in the generated PDF
                      </p>
                    </div>
                  )}
                </div>
